...
_min = min
_sum = sum
def _isfunc(arg): return isinstance( arg, ArrayFunc )
def _isevaluable( arg ): return isinstance( arg, Evaluable )
def _isscalar( arg ): return asarray(arg).ndim == 0
def _ascending( arg ): return ( numpy.diff(arg) > 0 ).all()
def _iszero( arg ): return isinstance( arg, Zeros ) or isinstance( arg, numpy.ndarray ) and numpy.all( arg == 0 )
def _isunit( arg ): return not _isfunc(arg) and ( numpy.asarray(arg) == 1 ).all()
def _subsnonesh( shape ): return tuple( 1 if sh is None else sh for sh in shape )
def _normdims( ndim, shapes ): return tuple( numeric.normdim(ndim,sh) for sh in shapes )
def _zeros( shape ): return Zeros( shape )
def _zeros_like( arr ): return _zeros( arr.shape )

# for consistency in Add and Multiply arguments: the smallest Evaluable first
def _issorted( a, b ): return not isinstance(b,Evaluable) or isinstance(a,Evaluable) and id(a) <= id(b)
def _sorted( a, b ): return (a,b) if _issorted(a,b) else (b,a)
...
def negative( arg ): return multiply( arg, -1 )
def nsymgrad( arg, coords ): return ( symgrad(arg,coords) * coords.normal() ).sum(-1)
def ngrad( arg, coords ): return ( grad(arg,coords) * coords.normal() ).sum(-1)
def sin( arg ): return pointwise( [arg], numpy.sin, cos )
def _cos_deriv( x ): return -sin(x)
def cos( arg ): return pointwise( [arg], numpy.cos, _cos_deriv )
def rotmat( arg ): return asarray( [[cos(arg),sin(arg)],[-sin(arg),cos(arg)]] )
def _tan_deriv( x ): return cos(x)**-2
def tan( arg ): return pointwise( [arg], numpy.tan, _tan_deriv )
def _arcsin_deriv( x ): return reciprocal(sqrt(1-x**2))
def arcsin( arg ): return pointwise( [arg], numpy.arcsin, _arcsin_deriv )
def _arccos_deriv( x ): return -reciprocal(sqrt(1-x**2))
def arccos( arg ): return pointwise( [arg], numpy.arccos, _arccos_deriv )
def exp( arg ): return pointwise( [arg], numpy.exp, exp )
def ln( arg ): return pointwise( [arg], numpy.log, reciprocal )
def log2( arg ): return ln(arg) / ln(2)
def log10( arg ): return ln(arg) / ln(10)
def sqrt( arg ): return power( arg, .5 )
def reciprocal( arg ): return power( arg, -1 )
def _argmin_func( *x ): return numpy.argmin(numeric.stack(x),axis=0)
def argmin( arg, axis ): return pointwise( bringforward(arg,axis), _argmin_func, _zeros_like )
def _argmax_func( *x ): return numpy.argmax(numeric.stack(x),axis=0)
def argmax( arg, axis ): return pointwise( bringforward(arg,axis), _argmax_func, _zeros_like )
def _arctan2_deriv( x ): return stack([x[1],-x[0]]) / sum(power(x,2),0)
def arctan2( arg1, arg2=None ): return pointwise( arg1 if arg2 is None else [arg1,arg2], numpy.arctan2, _arctan2_deriv )
def greater( arg1, arg2=None ): return pointwise( arg1 if arg2 is None else [arg1,arg2], numpy.greater, _zeros_like )
def less( arg1, arg2=None ): return pointwise( arg1 if arg2 is None else [arg1,arg2], numpy.less, _zeros_like )
def min( arg1, *args ): return choose( argmin( arg1 if not args else (arg1,)+args, axis=0 ), arg1 if not args else (arg1,)+args )
def max( arg1, *args ): return choose( argmax( arg1 if not args else (arg1,)+args, axis=0 ), arg1 if not args else (arg1,)+args )
def abs( arg ): return arg * sign(arg)
def sinh( arg ): return .5 * ( exp(arg) - exp(-arg) )
def cosh( arg ): return .5 * ( exp(arg) + exp(-arg) )
def tanh( arg ): return 1 - 2. / ( exp(2*arg) + 1 )
def arctanh( arg ): return .5 * ( ln(1+arg) - ln(1-arg) )
def piecewise( level, intervals, *funcs ): return choose( sum( greater( insert(level,-1), intervals ), -1 ), funcs )
def trace( arg, n1=-2, n2=-1 ): return sum( takediag( arg, n1, n2 ), -1 )
def eye( n ): return diagonalize( expand( [1.], (n,) ) )
def norm2( arg, axis=-1 ): return sqrt( sum( multiply( arg, arg ), axis ) )
def heaviside( arg ): return choose( greater( arg, 0 ), [0.,1.] )
def divide( arg1, arg2 ): return multiply( arg1, reciprocal(arg2) )
def subtract( arg1, arg2 ): return add( arg1, negative(arg2) )
def mean( arg ): return .5 * ( arg + opposite(arg) )
def jump( arg ): return arg - opposite(arg)
def add_T( arg, axes=(-2,-1) ): return swapaxes( arg, axes ) + arg
def edit( arg, f ): return arg._edit(f) if _isevaluable(arg) else arg
