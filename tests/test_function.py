...
class check(TestCase):
  ...
  def setUp(self):
    ...
    numpy.random.seed(0)
    self.args = [function.Polyval(numeric.dot(numpy.random.uniform(size=shape+poly.shape[:1], low=self.low, high=self.high), poly), function.rootcoords(domain.roots)) for shape in self.shapes]
    if self.pass_geom:
        self.args += [self.geom]
