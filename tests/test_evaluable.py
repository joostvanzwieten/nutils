...
class check(TestCase):
    ...
    for axis in range(self.op_args.ndim+1):
      with self.subTest(axis=axis):
        self.assertFunctionAlmostEqual(decimal=15,
          ...
          actual=evaluable.InsertAxis(self.op_args, axis=axis, length=2))
  ...
  def test_inflate(self):
    for iax, sh in enumerate(self.op_args.shape):
      ...
      self.assertFunctionAlmostEqual(decimal=15,
        desired=desired,
        actual=evaluable.Inflate(self.op_args, dofmap=dofmap, length=sh*2-1, axis=iax))
  ...
  def test_opposite(self):
    self.assertArrayAlmostEqual(decimal=14,
      desired=self.n_op(*self.ifacesmp.eval([evaluable.opposite(arg) for arg in self.args])),
      actual=self.ifacesmp.eval(evaluable.opposite(self.op_args)))

  def find(self, target, xi0):
    elemtrans, = self.sample.transforms[0]
    ndim, = self.geom.shape
    J = evaluable.localgradient(self.geom, ndim)
    Jinv = evaluable.inverse(J).prepare_eval()
    countdown = 5
    iiter = 0
  ...
  def test_localgradient(self):
    ...
    argsfun = evaluable.Tuple(self.args).prepare_eval()
    exact = self.sample.eval(evaluable.localgradient(self.op_args, ndims=self.ndim))
  ...
  def test_jacobian(self):
    ...
    for iarg, shape in enumerate(self.shapes):
      ...
      fx0, Jx0 = self.sample.eval([f, evaluable.derivative(f, x)], x=x0)
...
_check('inflate', lambda f: evaluable.Inflate(f,dofmap=evaluable.Guard([0,3]),length=4,axis=1), lambda a: numpy.concatenate([a[:,:,:1], numpy.zeros_like(a), a[:,:,1:]], axis=2), [(4,2,4)])
_check('inflate-constant', lambda f: evaluable.Inflate(f,dofmap=[0,3],length=4,axis=1), lambda a: numpy.concatenate([a[:,:,:1], numpy.zeros_like(a), a[:,:,1:]], axis=2), [(4,2,4)])
...
class blocks(TestCase):
  ...
  def test_multiply_equal(self):
    ((i,), f), = evaluable.multiply(evaluable.Inflate([1,2], dofmap=[0,2], length=3, axis=0), evaluable.Inflate([3,4], dofmap=[0,2], length=3, axis=0)).blocks
    self.assertEqual(i, evaluable.asarray([0,2]))
    self.assertAllEqual(f.eval(), [1*3,2*4])

  def test_multiply_embedded(self):
    ((i,), f), = evaluable.multiply([1,2,3], evaluable.Inflate([4,5], dofmap=[0,2], length=3, axis=0)).blocks
    self.assertEqual(i, evaluable.asarray([0,2]))
    self.assertAllEqual(f.eval(), [1*4,3*5])

  def test_multiply_overlapping(self):
    ((i,), f), = evaluable.multiply(evaluable.Inflate([1,2], dofmap=[0,1], length=3, axis=0), evaluable.Inflate([3,4], dofmap=[1,2], length=3, axis=0)).blocks
    self.assertEqual(i, evaluable.asarray([1]))
    self.assertAllEqual(f.eval(), 2*3)

  def test_multiply_disjoint(self):
    blocks = evaluable.multiply(evaluable.Inflate([1,2], dofmap=[0,2], length=4, axis=0), evaluable.Inflate([3,4], dofmap=[1,3], length=4, axis=0)).blocks
    self.assertEqual(blocks, ())

  def test_multiply_fallback(self):
    ((i,), f), = evaluable.multiply(evaluable.Inflate([1,2], dofmap=evaluable.Guard([0,1]), length=3, axis=0), evaluable.Inflate([3,4], dofmap=evaluable.Guard([1,2]), length=3, axis=0)).blocks
    self.assertEqual(i, evaluable.Range(3))
  ...
  def test_takediag_embedded_axis(self):
    ((i,), f), = evaluable.takediag(evaluable.Inflate([[1,2,3],[4,5,6]], dofmap=[0,2], length=3, axis=0)).blocks
    self.assertEqual(i, evaluable.asarray([0,2]))
    self.assertAllEqual(f.eval(), [1,6])

  def test_takediag_embedded_rmaxis(self):
    ((i,), f), = evaluable.takediag(evaluable.Inflate([[1,2],[3,4],[5,6]], dofmap=[0,2], length=3, axis=1)).blocks
    self.assertEqual(i, evaluable.asarray([0,2]))
    self.assertAllEqual(f.eval(), [1,6])

  def test_takediag_overlapping(self):
    ((i,), f), = evaluable.takediag(evaluable.Inflate(evaluable.Inflate([[1,2],[3,4]], dofmap=[0,1], length=3, axis=0), dofmap=[1,2], length=3, axis=1)).blocks
    self.assertEqual(i, evaluable.asarray([1]))
    self.assertAllEqual(f.eval(), 3)

  def test_takediag_disjoint(self):
    blocks = evaluable.takediag(evaluable.Inflate(evaluable.Inflate([[1,2],[3,4]], dofmap=[0,2], length=4, axis=0), dofmap=[1,3], length=4, axis=1)).blocks
    self.assertEqual(blocks, ())

  def test_takediag_fallback(self):
    ((i,), f), = evaluable.takediag(evaluable.Inflate(evaluable.Inflate([[1,2],[3,4]], dofmap=evaluable.Guard([0,1]), length=3, axis=0), dofmap=evaluable.Guard([1,2]), length=3, axis=1)).blocks
    self.assertEqual(i, evaluable.Range(3))
...
class sampled(TestCase):
  ...
  def test_isarray(self):
    self.assertTrue(evaluable.isarray(self.f_sampled))
...
class piecewise(TestCase):
  ...
  def setUp(self):
    ...
    x, = self.geom
    if self.partition:
      left, mid, right = evaluable.partition(x, .2, .8)
      self.f = left + evaluable.sin(x) * mid + x**2 * right
    else:
      self.f = evaluable.piecewise(x, [.2,.8], 1, evaluable.sin(x), x**2)
  ...
  def test_deriv(self):
    g_ = self.domain.sample('uniform', 4).eval(evaluable.grad(self.f, self.geom)) # x=.125, .375, .625, .875
...
class elemwise(TestCase):
  ...
  def test_derivative(self):
    self.assertTrue(evaluable.iszero(evaluable.localgradient(self.func, self.domain.ndims)))
  ...
  def test_deprecated_elemwise(self):
    with self.assertWarns(warnings.NutilsDeprecationWarning):
      func = evaluable.elemwise(self.domain.transforms, self.data)
...
class jacobian(TestCase):
  ...
  def setUp(self):
    ...
    arg = evaluable.Argument('dofs', [4])
    self.v = self.basis.dot(arg)
    self.X = (self.geom[numpy.newaxis,:] * [[0,1],[-self.v,0]]).sum(-1) # X_i = <x_1, -2 x_0>_i
    self.J = evaluable.J(self.X, None if self.delayed else 2)
    self.dJ = evaluable.derivative(self.J, arg)
  ...
  def test_zeroderivative(self):
    otherarg = evaluable.Argument('otherdofs', (10,))
    values = self.domain.sample('uniform', 2).eval(evaluable.derivative(self.dJ, otherarg))
