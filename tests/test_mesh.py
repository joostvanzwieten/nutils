...
class gmsh(TestCase):
  ...
  def test_refine(self):
    ...
    canonical = lambda transtuple: tuple(map(transform.canonical, transtuple))
    assert set(map(canonical, boundary1.transforms)) == set(map(canonical, boundary2.transforms))
