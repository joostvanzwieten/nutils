...
class Common:
  ...
  def test_index_with_tail_children(self):
    assert len(self.check) == len(self.checkrefs)
    for i, (trans, ref) in enumerate(zip(self.check, self.checkrefs)):
  ...
  def test_contains_with_tail(self):
    ...
    for i, (trans, ref) in enumerate(zip(self.check, self.checkrefs)):
      ...
      if any(t[-1].fromdims for t in trans):
        for etrans in nutils.transform.edge_transforms(trans, ref):
          for shuffle in lambda t: t, lambda t: tuple(map(nutils.transform.canonical, t)):
            self.assertTrue(self.seq.contains_with_tail(shuffle(etrans)))
...
class Edges:
  ...
  def test_index_with_tail_edges(self):
    assert len(self.check) == len(self.checkrefs)
    for i, (trans, ref) in enumerate(zip(self.check, self.checkrefs)):
      for etrans in nutils.transform.edge_transforms(trans, ref):
        for shuffle in lambda t: t, lambda t: tuple(map(nutils.transform.canonical, t)):
          self.assertEqual(self.seq.index_with_tail(shuffle(etrans)), (i, tuple(t[-1:] for t in etrans)))

X1 = nutils.function.Root('X1', 1)
X2 = nutils.function.Root('X2', 2)
Y1 = nutils.function.Root('Y1', 1)
...
class EmptyTransforms(TestCase, Common, Edges):
  def setUp(self):
    self.checktodims = 1,
    self.seq = nutils.transformseq.EmptyTransforms(self.checktodims)
    self.check = ()
...
class ChainedTransforms(TestCase, Common, Edges):
  def setUp(self):
    self.seq = nutils.transformseq.ChainedTransforms([nutils.transformseq.PlainTransforms([(s0,),(s1,)], 1, 1), nutils.transformseq.PlainTransforms([(s2,),(s3,)], 1, 1)], (1,))
...
class StructuredTransforms1D(TestCase, Common, Edges):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.DimAxis(1,4,False)], 0)
    self.check = ((s1,),),((s2,),),((s3,),)
    self.checkmissing = ((s0,),),((s4,),),((c0,),)
    self.checkrefs = nutils.elementseq.asreferences([line]*3, 1)
    self.checktodims = 1,
...
class StructuredTransforms1DRefined(TestCase, Common, Edges):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.DimAxis(1,4,False)], 1)
...
class StructuredTransforms1DLeft(TestCase, Common):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.BndAxis(3,3,0,False)], 0)
    self.check = ((s3,e1),),
...
class StructuredTransforms1DRight(TestCase, Common):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.BndAxis(3,3,0,True)], 0)
    self.check = ((s2,e0),),
...
class StructuredTransforms1DInterfacesLeft(TestCase, Common):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.IntAxis(0,4,0,False)], 0)
...
class StructuredTransforms1DInterfacesRight(TestCase, Common):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.IntAxis(0,4,0,True)], 0)
...
class StructuredTransforms1DPeriodicInterfacesLeft(TestCase, Common):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.PIntAxis(0,4,0,False)], 0)
...
class StructuredTransforms1DPeriodicInterfacesRight(TestCase, Common):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(1, [nutils.transformseq.PIntAxis(0,4,0,True)], 0)
...
class StructuredTransforms2D(TestCase, Common, Edges):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(2, [nutils.transformseq.DimAxis(0,2,False),nutils.transformseq.DimAxis(2,4,False)], 0)
...
class StructuredTransforms2DRefined(TestCase, Common, Edges):
  def setUp(self):
    self.seq = nutils.transformseq.StructuredTransforms(2, [nutils.transformseq.DimAxis(0,2,False),nutils.transformseq.DimAxis(2,4,False)], 1)
...
class exceptions(TestCase):
  ...
  def test_PlainTransforms_invalid_fromdims(self):
    with self.assertRaisesRegex(ValueError, 'expected transforms with fromdims=2, but got .*'):
      nutils.transformseq.PlainTransforms([(s0,),(s1,)], 2, 2)

  def test_PlainTransforms_multiple_fromdims(self):
    with self.assertRaisesRegex(ValueError, 'expected transforms with fromdims=2, but got .*'):
      nutils.transformseq.PlainTransforms([(s0,),(s00,)], 2, 2)
